...
def read_detector(results, drone_locations, drone_angles):
    ...
    # 检查每个无人机是否检测到坦克
    max_count = min(DRONE_COUNT, len(results), len(drone_locations), len(drone_angles))
    for i in range(max_count):
        if 0 in results[i][0].boxes.cls.tolist():
            ...
            drones_data.append((
                ...
                drone_locations[i][2],  # 无人机z坐标
                drone_angles[i],  # 无人机方向角度
                (u_center, v_center, score)  # 检测框中心与置信度
            ))

    # 如果至少有两个无人机检测到坦克，则估计坦克位置
    if len(drones_data) >= 1:
        tank_position = estimate_tank_from_detections(drones_data)
...
class DroneWebSocketServer:
    ...
    def HandleRequestFunc(self, image_1, image_2, drone_1_location, drone_2_location,
        ...
        参数:
        - image_1, image_2: PIL Image对象
        - drone_1_location, drone_2_location: [x, y, z] 坐标列表
        - drone_1_angle, drone_2_angle: 无人机方向角度（度）
        - tank_location: [x, y, z] 坐标列表

        返回:
        - [[drone_1_newlocation, angle_1], [drone_2_newlocation, angle_2]]
        """
        pathresult = []
        ###################################sunhongze###################################
        ...
        result2 = self.detector.predict(
            image_2,
            save=False)
        drone_infos = [[drone_1_location, drone_1_angle], [drone_2_location, drone_2_angle]]
        predict_tank_location = read_detector1([result1, result2], drone_infos)
    ...
    ###################################sunhongze###################################
    def bounding(self, image_1, image_2):
        result = []
        for i in range(DRONE_COUNT):
